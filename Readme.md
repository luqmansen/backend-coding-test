...
4. Run `npm test`
5. Run `npm start`
6. Run `npm run gen-doc` to generate swagger doc. Generated page can be viewed in `localhost:8010/docs` (server must be running)
7. Run `npm run test:load` to run the load test
8Hit the server to test health `curl localhost:8010/health` and expect a `200` response 
